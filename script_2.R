...
patient2 <- read.spot("data/chip2.txt", cy3=3, cy5=2, bg.cy3=5, bg.cy5=4, ids=1, header = TRUE, sep="\t")
patient3 <- read.spot("data/chip3.txt", cy3=3, cy5=2, bg.cy3=5, bg.cy5=4, ids=1, header = TRUE, sep="\t")
...
## Segundo a informação das funções normalize withinArray e normalizBetweeArray do limma:
...
MA_list_fit <- lmFit(MA)
MA_list_eBayes <- eBayes(MA_list_fit, proportion = 0.05)

# tabelas
table1<-topTable(MA_list_eBayes,number=20,adjust="BH", genelist = normalized_bg_patient1@spotData$Id,
                 sort.by = "B")
table1
table2 <- topTable(MA_list_eBayes,number=20,adjust="BH", genelist = normalized_bg_patient1@spotData$Id,
                   sort.by = "logFC")
write.table(table,"out.txt")

# Volcano plot
par(mfrow=c(1,1))
volcanoplot(MA_list_eBayes,highlight=20, style = "b-statistic")
abline(a= min(table$B), b = 0, col = 'black', lty = 2)

# valor negativo maior
logFC_min <- max(table2[table2$logFC < 0, "logFC"])

#valor positivo menor
logFC_max<- min(table2[table2$logFC > 0, "logFC"])

# acrescentar retas verticais ao gráfico
abline(v = -1)
abline(v = 1)
